...
Write-Host "=== Seedance Cinematic Desk : build EXE ==="

Get-Process -Name "SeedanceCinematicDesk" -ErrorAction SilentlyContinue | Stop-Process -Force
Start-Sleep -Seconds 2
$old = Join-Path $PSScriptRoot "dist\SeedanceCinematicDesk.exe"
if (Test-Path $old) {
  try { Remove-Item -LiteralPath $old -Force } catch {
    throw "Cannot replace dist\SeedanceCinematicDesk.exe. Close the running app first."
  }
}
...
& $vpy -m PyInstaller --noconfirm --clean SeedanceCinematicDesk.spec

$exe = Join-Path $PSScriptRoot "dist\SeedanceCinematicDesk.exe"
if (Test-Path $exe) {
  Write-Host "OK: $exe"
